...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (srtContent.trim().length === 0) {
      ...
      ffmpegCmd = `ffmpeg -y -i "${inputFilePath}" -vf "subtitles=${subtitleFilePath}:force_style='Alignment=2,FontName=Noto Sans CJK JP,FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H000000,BackColour=&H80000000,BorderStyle=3,Outline=2,Shadow=2,MarginV=35'" -c:a copy "${outputFilePath}"`;
    }
